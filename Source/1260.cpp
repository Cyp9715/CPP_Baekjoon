...
public:
    ...
    void DFS(int v)
    {
        ...
        // 조건식에 작은것부터 출력하도록 되어 있으므로, i=0 부터 loop 돌림.
        for (int i = 1; i <= N; i++)
        {
            // 현재 정점과 연결되어있고 방문되지 않았으면 → 방문처리.
            if (map[v][i] == 1 && visited[i] == 0)
            {
                // 진입하는 순간 재귀적으로 i가 출력됨.
    ...
    void run() 
    {
        ...
        // 입력을 받음과 동시에 map을 설정함.
        // 정점 연결 정보는 map[a][b]로 바로 접근 가능.
        // 메모리가 소폭 낭비되나, search를 하지 않아도 되기에 속도면에서 빠름.
        for (int i = 0; i < M; i++)
        {
            int a, b;
            std::cin >> a >> b;
